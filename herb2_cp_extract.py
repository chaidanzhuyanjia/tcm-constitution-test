#!/usr/bin/env python3
import csv, json, os, time, hashlib, zipfile
...
PIPE="https://gateway.pipeworx.io/v1/tools/herb_tcm_herb_detail"
OUT=Path("herb2_output"); RAW=OUT/"raw"; RAW.mkdir(parents=True,exist_ok=True)

def post_json(url,payload,timeout=90):
    r=requests.post(url,json=payload,timeout=timeout,headers={"User-Agent":"HERB2-CP-GHA/1.0","Content-Type":"application/json"})
    r.raise_for_status()
    return r.json(), r.content

def get_section(herb_id, section):
    rows=[]; offset=0; total=None
    while total is None or offset<total:
        payload={"id":herb_id,"category":"herb","limit":200,"offset":offset,"sections":[section]}
        obj,raw=post_json(PIPE,payload)
        stamp=f"{herb_id}_{section}_{offset:06d}.json"
        (RAW/stamp).write_bytes(raw)
        sec=obj.get(section) or (obj.get("result") or {}).get(section)
        if sec is None:
            raise RuntimeError(f"{herb_id} missing section {section}: {str(obj)[:500]}")
        total=int(sec.get("total",0)); batch=sec.get("rows") or []
        rows.extend(batch)
        if not sec.get("truncated"): break
        if not batch: raise RuntimeError(f"{herb_id} {section} truncated but empty page at {offset}")
        offset += len(batch)
        time.sleep(0.25)
    return rows
...
def pick_gene(row):
    for k,v in row.items():
        kl=k.lower()
        if ("gene" in kl or "target.name" in kl or kl in {"target name","target"}) and isinstance(v,str) and v and not v.startswith("HBTAR"):
            return v
    return ""
...
hi_edges=[]; it_edges=[]; paths=[]; summary=[]; errors={}
for formula,ids in HERBS.items():
    for hid in ids:
        try:
            hi=get_section(hid,"herb_ingredient")
            it=get_section(hid,"ingredient_target")
...
qc={"errors":errors,"summary":summary,"primary_ready":not errors and all(x["unique_targets"]>=5 and x["herbs_with_paths"]>0 for x in summary)}
(OUT/"QC_report.json").write_text(json.dumps(qc,ensure_ascii=False,indent=2),encoding="utf-8")
manifest={"source":"HERB 2.0 live via Pipeworx herb_tcm_herb_detail","tool_url":PIPE,"frozen_herbs":HERBS,"retrieved_utc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())}
